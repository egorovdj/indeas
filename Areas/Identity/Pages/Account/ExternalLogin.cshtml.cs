...
namespace indeas.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("{Name} вошел в систему с помощью {LoginProvider} провайдера.", info.Principal.Identity.Name, info.LoginProvider);
        ...
        private IdentityUser CreateUser()
        {
            ...
            catch
            {
                throw new InvalidOperationException($"Не удается создать экземпляр '{nameof(IdentityUser)}'. " +
                    $"Убедитесь, что '{nameof(IdentityUser)}' не является абстрактным классом и имеет конструктор без параметров, или в качестве альтернативы" +
